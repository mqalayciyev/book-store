...
class CustomerController extends Controller
{
    ...
    {
        return view('manage.pages.customer.index');
    }

    public function index_data()
    {
        $rows = Customer::leftJoin('supplier_product', 'customer.id', '=', 'supplier_product.supplier_id')
            ->leftJoin('product', 'product.id', '=', 'supplier_product.product_id')
            ->select([
                'customer.*',
                DB::raw('count(supplier_product.supplier_id) as supplier_products')
            ]);
        $rows->groupBy('supplier_product.supplier_id', 'customer.id');
        return DataTables::eloquent($rows)
            ->addColumn('action', function ($row) {
                ...
                return '<div>
                <a href="' . route('manage.customer.edit', $row->id) . '" class="btn btn-xs btn-primary edit"> <i class="fa fa-edit"></i> ' . __('admin.Edit') . '</a>
    ...
    public function form($id = 0)
    {
        ...
        return view('manage.pages.customer.form', compact('entry'));
    }
    ...
    public function save($id = 0)
    {
        ...
        if ($id > 0) {
            $entry = Customer::where('id', $id)->firstOrFail();
            $entry->update($data);
        } else {
            $entry = Customer::create($data);
        }

        return redirect()
            ->route('manage.customer.edit', $entry->id)
    ...
    {
        $rows = Customer::find($request->input('id'));
        if ($rows->delete()) {
    ...
    public function mass_remove(Request $request)
    {
        ...
        $rows = Customer::whereIn('id', $id_array);
        if ($rows->delete()) {
            echo __('admin.Data Deleted');
        }
    }
