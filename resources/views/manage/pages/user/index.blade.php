...
@if (@$manage == 2)
    ...
    <section class="content-header">
        <h1 class="pull-left">@lang('admin.Users')</h1>
        <div class="pull-right">
            <a href="{{ route('manage.user.new') }}" class="btn btn-success">@lang('admin.Add New User')</a>
        </div>
    ...
    <script>
        $(function () {
            ...
            $('#index_table').DataTable({
                ...
                ordering: true,
                paging: true,
                ajax: '{{ route('manage.user.index_data')}}',
                columns: [
                    {data: 'name', name: 'name'},
                    {data: '{{ (auth('manage')->user()->is_manage == 2) ? 'name' : 'mobile'}}'},
                    {data: '{{ (auth('manage')->user()->is_manage == 2) ? 'name' : 'email'}}'},
                    {data: 'is_active'},
                    {data: 'updated_at'},
            ...
            $(document).on('click', '.delete', function () {
                ...
                if (confirm('{{ __('admin.Are you sure you want to delete this data?') }}')) {
                    $.ajax({
                        url: '{{ route('manage.user.delete_data') }}',
                        method: 'GET',
                        data: {id: id},
            ...
            $(document).on('click', '#bulk_delete', function () {
                ...
                if (confirm('{{ __('admin.Are you sure you want to delete this data?') }}')) {
                    ...
                    if (id.length > 0) {
                        $.ajax({
                            url: '{{ route('manage.user.mass_remove') }}',
                            method: 'GET',
                            data: {id: id},
                            success: function (data) {
                                alert(data);
